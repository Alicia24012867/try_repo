...
- 两个 assignment 共享完全相同的 `baseline_ref` 和
  ...
  若全部 probe 都有身份匹配的 review、但没有任何一项同时通过 build、全量 CEC、
  correctness-backed QoR 与 eligible review decision，canonical `winner.json` 的
  `winner` 为 `null`。该 `no_eligible_probe` 是已结算负验证结果：只把
  `outcome.json`、代表性 review/CEC/失败 patch 作为 diagnostic context，不把失败 QoR 放入
  winner/frontier，不更新 baseline，也不 exact replay；原子 Planning refresh 后仍启动同轮
  Flow 与 Logic。只有缺失、未完成、陈旧、schema/lineage 不符或被篡改的 batch 才阻塞两分支。
...
当前工作流先产生真实、CEC-backed 的 AIG 结构，并在每条 flow 后接入
`ASAP7_7nm_LVT_FF` 的映射、门级 sizing 与 STA：
`read_lib; map; topo; upsize; dnsize; topo; stime`。八条论文静态 flow 与补充的
`ftune` MAB lane 都保留逐 flow 物理面积、临界路径延迟及 ADP 证据；AIG nodes/depth
只是辅助反馈。冻结合同统一使用 1000 ps（1 GHz）参考周期，因而从物理延迟导出 WNS；
该周期是项目参考约束，不能冒充论文未公开的原始 SDC。Mapper Agent 尚未加入双 agent
写域，因此结果仍不能宣称复现论文所有三类 agent 的消融；但不再将 AIG node/depth
代理冒充 ASAP7 timing/area 指标。
