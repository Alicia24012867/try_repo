...
只是辅助反馈。WNS 不会凭空生成：只有在冻结 `asap7_qor.clock_period_ps` 时才由物理
延迟导出。Mapper Agent 尚未加入双 agent 写域，因此结果仍不能宣称复现论文所有三类
agent 的消融；但不再将 AIG node/depth 代理冒充 ASAP7 timing/area 指标。
